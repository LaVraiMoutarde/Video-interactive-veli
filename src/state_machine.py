        ...
        if self.state == InteractionState.TRACKING_LOST:
            if has_person:
                ...
                if elapsed >= self.tracking_reset_delay_s:
                    self.reset_session()
                    return StateOutput(self.state, None, 0.0, countdown_s=0.0, info_message="session reset")

                countdown = max(0.0, self.tracking_reset_delay_s - elapsed)
                return StateOutput(self.state, None, 0.0, countdown_s=countdown, info_message="tracking lost")
        ...
        if self.state == InteractionState.WAITING_START:
            ...
            if distance_m is not None:
                ...
                if launch_window_active:
                    info_message = "pret : levez les mains pour demarrer"
                elif distance_m > self.launch_max_distance_m:
                    info_message = "trop loin : rapprochez-vous pour demarrer"
                elif distance_m < self.launch_min_distance_m:
                    info_message = "trop proche : reculez pour demarrer"
                else:
                    info_message = "stabilizing near launch edge"
            else:
                launch_window_active = self._launch_window_active
                if has_person:
                    info_message = "distance stabilizing"
            ...
            if gestures.both_hands_triggered:
                ...
                if distance_for_start is None:
                    info_message = "distance unavailable"
                    ...
                    info_message = "video 1 started"
                else:
                    info_message = (
                        f"start denied: stand between {self.launch_min_distance_m:.1f}m and {self.launch_max_distance_m:.1f}m"
                    )
            ...
            if distance_m is not None:
                ...
                if distance_m <= self._switch_enter_distance():
                    ...
                    info_message = "video 2 gesture zone"
            ...
            if distance_m is not None:
                ...
                if distance_m >= self._switch_exit_distance():
                    self.state = InteractionState.VIDEO1_CONTROL
                    info_message = "back to video 1 control"
                elif not video2_trigger_window_active:
                    info_message = "adjust distance for right-hand trigger"
                    ...
                    if video2_trigger_window_active:
                        ...
                        info_message = "video 2 started"
                    else:
                        info_message = (
                            "switch denied: right hand valid only between "
                            f"{self.video2_trigger_min_distance_m:.1f}m and {self.video2_trigger_max_distance_m:.1f}m"
                        )
